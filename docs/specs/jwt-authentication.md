...
**An email only counts as "already registered" once it's verified — an abandoned PENDING_VERIFICATION signup doesn't block a retry.**
`UserService.registerUser` throws `409 USER_ALREADY_EXISTS` only when the existing row for that email has `emailVerifiedAt` set. If someone registers, never completes OTP verification, and submits `POST /auth/register` again with the same email, the existing `PENDING_VERIFICATION` row is overwritten in place (same `id`, fresh trial window) instead of being blocked — otherwise a typo'd OTP or an abandoned signup would permanently squat the email with no way back in short of contacting support. No new column was added for this: `email_verified_at` (already on `users`, set by `OtpService.verifyOtp` the moment a `PENDING_VERIFICATION` user verifies) already captures "is this email verified," so its null/non-null state is used directly rather than introducing a redundant boolean that could drift out of sync with it.
...
- **`POST /auth/register`** — `RegisterRequest {firstName, lastName, companyName, jobTitle?, country?, email}` → `201` `OtpRequestResponse {message, expiresInSeconds}`. Creates a `PENDING_VERIFICATION` user and sends the first OTP — or, if a `PENDING_VERIFICATION` (never-verified) row already exists for that email, overwrites it in place and sends a fresh OTP. `409 USER_ALREADY_EXISTS` only if the email belongs to an already-*verified* account (`emailVerifiedAt` set).
...
`LoginResponse`: `TokenResponse` plus `user: UserResponse` (`{id, email, firstName, lastName, status, displayName?, roles?, trialStartDate?, trialEndDate?}` — `firstName`/`lastName` are the registration fields, always present; the trial dates are UTC ISO-8601 instants, mapped from the `User` entity's `Instant` columns via `UserResponseMapper`, shared with `GET/PUT /users/me`) plus `firstLogin: boolean`. Only `/auth/otp/verify` returns this wider shape; `/auth/tokens` and `/auth/refresh` keep returning plain `TokenResponse` since they're not the "just logged in, populate the dashboard" moment.
...
- 2026-08-26 — Added `firstLogin: boolean` to `LoginResponse` (`AuthController.toLoginResponse`, sibling to `user`, not part of `UserResponse` itself), wired to the pre-existing but previously-unused `users.first_login` column so the portal can drive the guided tour exactly once per account. `AuthService.issueTokensForVerifiedEmail` captures `user.isFirstLogin()` into a local `boolean` *before* calling the new `UserService.clearFirstLoginFlag(user)` (sets `firstLogin = false` and saves, no-op if already false) — capturing into a primitive first, rather than reading the flag off the entity after clearing it, is what lets the same login response report `true` on the one login that earns it while the persisted row is already flipped for every login after. `LoginResult` gained a third `firstLogin` field to carry that captured value through to the controller. Both `/auth/otp/verify` and the interim `/auth/tokens` share `issueTokensForVerifiedEmail`, so either one consumes the flag — acceptable since `/auth/tokens` is non-prod-only and manual-testing-only.
- 2026-08-25 — Added `theme` (`light`/`dark`, `users.theme VARCHAR(10) NOT NULL DEFAULT 'LIGHT'`, migration `V5__add_theme_to_users_table.sql`) so the portal's theme toggle persists per-user instead of only in `localStorage`. `UserResponse.theme` is required (always present); `UpdateUserRequest.theme` is optional. `UserService.updateDisplayName` was renamed to `updateProfile(id, displayName, theme)` and now only touches fields that are non-null — `PUT /users/me` is a partial update, not a full-replace, so toggling the theme must not wipe out a previously-set `displayName` (and vice versa) just because the caller didn't resend it; this was a latent gap in the old `updateDisplayName`, not something specific to theme. On the portal: `Theme` (`core/theme.ts`) now syncs both ways — `toggle()` calls `PUT /users/me` when authenticated (fire-and-forget, the UI already updated optimistically via the local signal), and an `effect()` adopts the signed-in user's saved theme on login/session-restore, overriding whatever was in `localStorage` on that device. Anonymous/logged-out toggling is unchanged (`localStorage` only).
- 2026-08-25 — `POST /auth/register` no longer 409s for an email stuck in `PENDING_VERIFICATION` (registered but never OTP-verified) — it overwrites that row in place and sends a fresh code. Only a *verified* email (`emailVerifiedAt` set) now blocks re-registration. Deliberately reused the existing `email_verified_at` column for this check rather than adding a new boolean — it already carries exactly that state, set by `OtpService.verifyOtp`.
- 2026-08-24 — CORS is configured in `config/CorsConfig.java`: a `CorsConfigurationSource` bean allowing all origins (`setAllowedOriginPatterns(List.of("*"))`), all methods, and all headers, with `allowCredentials(false)`. `SecurityConfig` picks it up via `.cors(Customizer.withDefaults())`. This is defensible for a pure Bearer-token API — with no cookie-based auth and credentials disabled, an open origin list carries no CSRF exposure — but it's broader than an explicit allowlist; worth the team confirming that's intentional before this reaches an environment where it matters.
