...
- **`POST /auth/otp/verify`** — `OtpVerifyRequest {email, code}` → `TokenResponse`. Verifies the code; if the user was `PENDING_VERIFICATION`, activates the account first. Either way, issues a real JWT token pair — this is the endpoint both registration-completion and login converge on.
