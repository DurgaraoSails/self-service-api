...
- An existing POC can be redeployed at a new version on demand (`deploy-new-version`), with an explicit or default (minor) semver bump. (Both the `bump` parameter and the minor default were dropped before this shipped — the endpoint became `POST /pocs/{id}/deploy` with no body, allocating a patch bump from this platform's own rows. Where the number comes from is superseded again by `poc-tag-driven-deployment.md`.)
...
The admin never supplies a tag: `deploy-new-version` with no body bumps the minor version off whatever tags already exist. **This was never accurate about the shipped code**, and the gap is exactly the defect `poc-tag-driven-deployment.md` exists to close: the implementation bumped a patch off this platform's own `poc_versions` rows and never read the repository's tags at all, so a repository that already had tags became undeployable. That spec restores the behaviour described here — the number derived from the tags git actually has — and additionally lets an admin pick an existing tag rather than always creating one. So the UI's "Update" button is just `check-updates` (to decide whether to offer it) followed by an unparameterized `deploy-new-version`. The optional `bump` field stays in the contract for a future power-user path, but nothing in the intended flow sends it. One consequence worth knowing: `updateAvailable: false` is returned both when a POC is genuinely up to date and when nothing has ever gone live — the UI distinguishes them via `deployedCommitSha == null`, since "never deployed" should offer *Deploy*, not *Update*.
